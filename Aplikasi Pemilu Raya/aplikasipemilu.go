// Caroline Carren Aurelia R
// 2311102174
// Aplikasi Pemilu Raya 2025
...
// Ganti nama struct dan variabel untuk menghindari konflik
type namaWilayah struct {
	A, B, C string
}

var wilayah = namaWilayah{
	A: "Provinsi A",
	B: "Provinsi B",
	C: "Provinsi C",
}
...
func menuKPU(A *date, TA *arrProvA, TB *arrProvB, TC *arrProvC, nA, nB, nC, nPA, nPB, nPC *int) {
	...
	fmt.Println("7. Atur Nama Provinsi")
	...
	case 6:
		...
		menuKPU(A, TA, TB, TC, nA, nB, nC, nPA, nPB, nPC)
	case 7:
		aturNamaProvinsi(A, TA, TB, TC, nA, nB, nC, nPA, nPB, nPC)
	case 8:
		menuUtama(A, TA, TB, TC, nA, nB, nC, nPA, nPB, nPC)
	}
}
...
func hasilPEMILU(TA arrProvA, TB arrProvB, TC arrProvC, nA, nB, nC, nPA, nPB, nPC int) {
	...
	// Tampilkan statistik sebelum hasil detail
	tampilkanStatistik(TA, TB, TC, nPA, nPB, nPC)

	hitungSemuaCALON(TA, TB, TC, nA, nB, nC, nPA, nPB, nPC)
	tampilkanAmbangBatas(TA, TB, TC, nA, nB, nC)
}

// Fungsi tampilkanStatistik yang sudah ada, dengan perbaikan format
func tampilkanStatistik(TA arrProvA, TB arrProvB, TC arrProvC, nPA, nPB, nPC int) {
	totalPemilih := nPA + nPB + nPC

	fmt.Println("\n============================================")
	fmt.Println("           STATISTIK PEMILU                ")
	fmt.Println("--------------------------------------------")
	fmt.Printf("Total Pemilih: %d orang\n", totalPemilih)

	if totalPemilih > 0 {
		fmt.Printf("Provinsi A: %d pemilih (%.1f%%)\n", nPA, float64(nPA)/float64(totalPemilih)*100)
		fmt.Printf("Provinsi B: %d pemilih (%.1f%%)\n", nPB, float64(nPB)/float64(totalPemilih)*100)
		fmt.Printf("Provinsi C: %d pemilih (%.1f%%)\n", nPC, float64(nPC)/float64(totalPemilih)*100)
	} else {
		fmt.Println("Belum ada pemilih yang terdaftar")
	}
	fmt.Println("============================================\n")
}
...
// Tambahkan fungsi sequential search
...
// Fungsi aturNamaProvinsi yang diperbarui
func aturNamaProvinsi(A *date, TA *arrProvA, TB *arrProvB, TC *arrProvC, nA, nB, nC, nPA, nPB, nPC *int) {
	reader := bufio.NewReader(os.Stdin)

	fmt.Println("\n=================================")
	fmt.Println("    PENGATURAN NAMA PROVINSI     ")
	fmt.Println("=================================")

	fmt.Printf("\nNama provinsi saat ini:\n")
	fmt.Printf("1. %s\n", wilayah.A)
	fmt.Printf("2. %s\n", wilayah.B)
	fmt.Printf("3. %s\n", wilayah.C)

	fmt.Println("\nMasukkan nama provinsi baru:")

	fmt.Print("Provinsi 1: ")
	nama1, _ := reader.ReadString('\n')
	nama1 = strings.TrimSpace(nama1)
	if nama1 != "" {
		wilayah.A = nama1
	}

	fmt.Print("Provinsi 2: ")
	nama2, _ := reader.ReadString('\n')
	nama2 = strings.TrimSpace(nama2)
	if nama2 != "" {
		wilayah.B = nama2
	}

	fmt.Print("Provinsi 3: ")
	nama3, _ := reader.ReadString('\n')
	nama3 = strings.TrimSpace(nama3)
	if nama3 != "" {
		wilayah.C = nama3
	}

	fmt.Println("\nNama provinsi berhasil diperbarui!")
	menuKPU(A, TA, TB, TC, nA, nB, nC, nPA, nPB, nPC)
}

// Fungsi utama untuk menampilkan statistik
func tampilkanStatistikLengkap(TA arrProvA, TB arrProvB, TC arrProvC, nA, nB, nC, nPA, nPB, nPC int) {
	// Header utama
	tampilkanHeaderStatistik()

	// 1. Tampilkan statistik partisipasi
	tampilkanStatistikPartisipasi(nPA, nPB, nPC)

	// 2. Tampilkan perolehan suara per provinsi
	tampilkanPerolehanSuara(TA, TB, TC, nA, nB, nC, nPA, nPB, nPC)

	// 3. Tampilkan statistik partai
	tampilkanStatistikPartai(TA, TB, TC, nA, nB, nC)

	// 4. Tampilkan grafik perolehan suara
	tampilkanGrafikSuara(TA, TB, TC, nA, nB, nC)

	// 5. Tampilkan ringkasan
	tampilkanRingkasanStatistik(TA, TB, TC, nA, nB, nC, nPA, nPB, nPC)
}

// Fungsi untuk menampilkan header statistik
func tampilkanHeaderStatistik() {
	fmt.Println("\n====================================")
	fmt.Println("        STATISTIK PEMILU           ")
	fmt.Println("====================================")
}

// Fungsi untuk menampilkan statistik partisipasi
func tampilkanStatistikPartisipasi(nPA, nPB, nPC int) {
	fmt.Println("\n[1. STATISTIK PARTISIPASI]")
	fmt.Println("============================================")

	totalMemilih := nPA + nPB + nPC
	totalTerdaftar := 200 // Contoh data

	fmt.Printf("Total Pemilih Terdaftar : %d orang\n", totalTerdaftar)
	fmt.Printf("Total yang Sudah Memilih: %d orang\n", totalMemilih)
	fmt.Printf("Persentase Partisipasi  : %.1f%%\n\n", float64(totalMemilih)/float64(totalTerdaftar)*100)

	// Detail per provinsi
	fmt.Println("Detail per Provinsi:")
	fmt.Printf("Provinsi A: %d pemilih (%.1f%%)\n", nPA, float64(nPA)/float64(totalMemilih)*100)
	fmt.Printf("Provinsi B: %d pemilih (%.1f%%)\n", nPB, float64(nPB)/float64(totalMemilih)*100)
	fmt.Printf("Provinsi C: %d pemilih (%.1f%%)\n", nPC, float64(nPC)/float64(totalMemilih)*100)
}

// Fungsi untuk menampilkan perolehan suara
func tampilkanPerolehanSuara(TA arrProvA, TB arrProvB, TC arrProvC, nA, nB, nC, nPA, nPB, nPC int) {
	fmt.Println("\n[2. PEROLEHAN SUARA]")
	fmt.Println("============================================")

	// Provinsi A
	fmt.Println("\nProvinsi A:")
	fmt.Printf("Total Suara: %d\n", nPA)
	fmt.Println("--------------------------------------------")
	for i := 0; i < nA; i++ {
		persentase := float64(TA[i].totalSuaraCalon) / float64(nPA) * 100
		fmt.Printf("%d. %s - %s: %d suara (%.1f%%)\n",
			i+1,
			TA[i].calon.nama,
			TA[i].calon.partai,
	...
	fmt.Println("--------------------------------------------")
	for i := 0; i < nB; i++ {
		persentase := float64(TB[i].totalSuaraCalon) / float64(nPB) * 100
		fmt.Printf("%d. %s - %s: %d suara (%.1f%%)\n",
			i+1,
			TB[i].calon.nama,
			TB[i].calon.partai,
	...
	fmt.Println("--------------------------------------------")
	for i := 0; i < nC; i++ {
		persentase := float64(TC[i].totalSuaraCalon) / float64(nPC) * 100
		fmt.Printf("%d. %s - %s: %d suara (%.1f%%)\n",
			i+1,
			TC[i].calon.nama,
			TC[i].calon.partai,
...
// Struktur untuk menyimpan data partai terurut
type DataPartai struct {
	nama  string
	suara int
}

// Fungsi untuk menampilkan statistik partai (diperbaiki)
func tampilkanStatistikPartai(TA arrProvA, TB arrProvB, TC arrProvC, nA, nB, nC int) {
	fmt.Println("\n[3. STATISTIK PARTAI]")
	fmt.Println("============================================")

	// Gunakan slice untuk menjaga urutan
	var dataPartai []DataPartai
	partaiSuara := make(map[string]int)
	totalSuara := 0

	// Hitung total suara
	for i := 0; i < nA; i++ {
		partaiSuara[TA[i].calon.partai] += TA[i].totalSuaraCalon
		totalSuara += TA[i].totalSuaraCalon
	}
	for i := 0; i < nB; i++ {
		partaiSuara[TB[i].calon.partai] += TB[i].totalSuaraCalon
		totalSuara += TB[i].totalSuaraCalon
	}
	for i := 0; i < nC; i++ {
		partaiSuara[TC[i].calon.partai] += TC[i].totalSuaraCalon
		totalSuara += TC[i].totalSuaraCalon
	}

	// Convert map ke slice untuk pengurutan
	for partai, suara := range partaiSuara {
		dataPartai = append(dataPartai, DataPartai{partai, suara})
	}

	// Urutkan berdasarkan suara (descending)
	sort.Slice(dataPartai, func(i, j int) bool {
		return dataPartai[i].suara > dataPartai[j].suara
	})

	// Tampilkan hasil terurut
	fmt.Println("\nTotal Suara per Partai:")
	for _, data := range dataPartai {
		if totalSuara > 0 { // Hindari division by zero
			persentase := float64(data.suara) / float64(totalSuara) * 100
			fmt.Printf("%s: %d suara (%.1f%%)\n", data.nama, data.suara, persentase)
		} else {
			fmt.Printf("%s: %d suara (0.0%%)\n", data.nama, data.suara)
		}
	}
}

// Fungsi untuk membuat grafik (diperbaiki)
func buatGrafik(jumlah, maxSuara int) string {
	const maxLength = 50 // Panjang maksimum grafik
	bar := ""
	if maxSuara > 0 {
		length := int(float64(jumlah) / float64(maxSuara) * float64(maxLength))
		for i := 0; i < length; i++ {
			bar += "█"
		}
	}
...
// Fungsi untuk menampilkan grafik suara (diperbaiki)
func tampilkanGrafikSuara(TA arrProvA, TB arrProvB, TC arrProvC, nA, nB, nC int) {
	fmt.Println("\n[4. GRAFIK PEROLEHAN SUARA]")
	fmt.Println("============================================")

	// Temukan suara terbanyak untuk skala grafik
	maxSuara := 0
	for i := 0; i < nA; i++ {
		if TA[i].totalSuaraCalon > maxSuara {
			maxSuara = TA[i].totalSuaraCalon
		}
	}
	for i := 0; i < nB; i++ {
		if TB[i].totalSuaraCalon > maxSuara {
			maxSuara = TB[i].totalSuaraCalon
		}
	}
	for i := 0; i < nC; i++ {
		if TC[i].totalSuaraCalon > maxSuara {
			maxSuara = TC[i].totalSuaraCalon
		}
	}

	// Tampilkan grafik dengan skala yang tepat
	fmt.Println("\nProvinsi A:")
	for i := 0; i < nA; i++ {
		fmt.Printf("%-15s: %s (%d)\n",
			TA[i].calon.nama,
			buatGrafik(TA[i].totalSuaraCalon, maxSuara),
			TA[i].totalSuaraCalon)
	}
	...
	for i := 0; i < nB; i++ {
		fmt.Printf("%-15s: %s (%d)\n",
			TB[i].calon.nama,
			buatGrafik(TB[i].totalSuaraCalon, maxSuara),
			TB[i].totalSuaraCalon)
	}
	...
	for i := 0; i < nC; i++ {
		fmt.Printf("%-15s: %s (%d)\n",
			TC[i].calon.nama,
			buatGrafik(TC[i].totalSuaraCalon, maxSuara),
			TC[i].totalSuaraCalon)
	}
}

// Fungsi untuk menampilkan persentase dengan validasi
func hitungPersentase(bagian, total int) float64 {
	if total > 0 {
...
// Fungsi untuk menampilkan ringkasan statistik
func tampilkanRingkasanStatistik(TA arrProvA, TB arrProvB, TC arrProvC, nA, nB, nC, nPA, nPB, nPC int) {
	...
	// Hitung partai dengan suara terbanyak
	partaiSuara := make(map[string]int)
	for i := 0; i < nA; i++ {
		partaiSuara[TA[i].calon.partai] += TA[i].totalSuaraCalon
	}
	...
	var partaiTerbanyak string
	var suaraTerbanyak int
	for partai, suara := range partaiSuara {
		if suara > suaraTerbanyak {
			suaraTerbanyak = suara
			partaiTerbanyak = partai
		}
	}

	fmt.Printf("1. Tingkat Partisipasi : %.1f%%\n", float64(totalPemilih)/float64(totalTerdaftar)*100)
	fmt.Printf("2. Partai Terbanyak    : %s (%d suara)\n", partaiTerbanyak, suaraTerbanyak)
	fmt.Printf("3. Provinsi Tertinggi   : %s (%d pemilih)\n",
		func() string {
			if nPA >= nPB && nPA >= nPC {
				return "Provinsi A"
			} else if nPB >= nPA && nPB >= nPC {
				return "Provinsi B"
			}
			return "Provinsi C"
		}(),
...
// Menu statistik
func menuStatistik(TA *arrProvA, TB *arrProvB, TC *arrProvC, nA, nB, nC, nPA, nPB, nPC *int) {
	for {
		fmt.Println("\n====================================")
		fmt.Println("         MENU STATISTIK             ")
		fmt.Println("====================================")
		fmt.Println("1. Tampilkan Semua Statistik")
		fmt.Println("2. Statistik Partisipasi")
		fmt.Println("3. Perolehan Suara")
		fmt.Println("4. Statistik Partai")
		fmt.Println("5. Grafik Suara")
		fmt.Println("6. Ringkasan")
		...
		switch pilihan {
		case 1:
			tampilkanStatistikLengkap(*TA, *TB, *TC, *nA, *nB, *nC, *nPA, *nPB, *nPC)
		case 2:
			tampilkanStatistikPartisipasi(*nPA, *nPB, *nPC)
		case 3:
			tampilkanPerolehanSuara(*TA, *TB, *TC, *nA, *nB, *nC, *nPA, *nPB, *nPC)
